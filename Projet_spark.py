...
if __name__ == "__main__" :
  ...
  # Filter par le nom et le temps en 5 secondes
  query = df.groupBy(window("timestamp", "5 seconds"), "name").count()
  query = query.filter(query["count"] > 7)
  query = query.withColumn('message', lit('trop de message'))
